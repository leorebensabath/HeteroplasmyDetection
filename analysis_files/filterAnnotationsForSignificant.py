...
df_summary = pd.DataFrame(columns = ['Location', 'Consequence'], dtype = 'int')
        ...
        if len(files) == 4 :
            ...
            for file in files :
                with open(file, "rb") as f:
                    ...
                    locations_pos = df["location"][:i_case+j_control][df['stat_value'][:i_case+j_control]>0]
            ...
            new_vcf = []
            print(significantLocations)

            with open(f'/oak/stanford/groups/euan/projects/leore_mtdna/data_pass1B/annotatedVcfTissue_everything/{tissue}_{sex}.filtered', 'r') as file:
                for line in file :
                    if line[0] == '#' :
                        new_vcf.append(line)
                    if line[0] != '#':
                        ...
                        if str(location) in significantLocations :
                            new_vcf.append(line)

            with open(f'/oak/stanford/groups/euan/projects/leore_mtdna/data_pass1B/annotatedVcfTissue_everything_significant/{tissue}_{sex}.filtered', 'w') as file:
                for line in new_vcf :
                    file.write(line)
